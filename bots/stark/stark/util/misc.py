...
def check_filters(filters: Union[Filter, CustomFilter], Stark: "Stark") -> None:
    """Recursively check filters to set :obj:`~Stark` into :obj:`~CustomFilter` if needed"""
    if isinstance(filters, (AndFilter, OrFilter, InvertFilter)):
        check_filters(filters.base, Stark)
    if isinstance(filters, (AndFilter, OrFilter)):
        check_filters(filters.other, Stark)
