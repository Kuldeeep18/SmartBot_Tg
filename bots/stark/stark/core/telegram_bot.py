...
from stark.language import get_lang_file
from stark.util.cache_limiter import CacheLimiter

from .stark_mixin_base import MixinBase
from .sqlite_storage import SQLiteStorage

if TYPE_CHECKING:
    from .stark_bot import stark
...
class TelegramBot(MixinBase):
    ...
    def __init__(self: "Stark", **kwargs: Any) -> None:
    ...
    async def init_client(self: "Stark") -> None:
        ...
        if "--fresh" not in sys.argv:
            ...
            file = AsyncPath("Stark/Stark.session")
        ...
        self.client = Client(
            ...
            storage=SQLiteStorage("Stark"),
        )

    async def start(self: "Stark") -> None:
        if self.__running:
    ...
    async def idle(self: "Stark") -> None:
        if self.__running:
            raise RuntimeError("This bot instance is already running")
    ...
    def update_plugin_events(self: "Stark") -> None:
    ...
    @property
    def events_activated(self: "Stark") -> int:
        return len(self._plugin_event_handlers)

    def redact_message(self: "Stark", text: str) -> str:
    ...
    @cached(ttl=60)
    async def get_chat(self: "Stark", chat_id: int) -> Union[Chat, ChatPreview]:
        """Wrapper for `Client.get_chat` with a TTL cache."""
        return await self.client.get_chat(chat_id)

    @cached(ttl=60)
    async def get_chat_member(self: "Stark", chat_id: int, user_id: int) -> ChatMember:
